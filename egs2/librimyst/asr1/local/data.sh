...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    if [ ! -e "${MYST}/data" ]; then
        log "stage 1: ${MYST}/data doesn't exist!"
        exit 1
	# echo "stage 1: Data Download to ${LIBRISPEECH}"
	# for part in dev-clean test-clean dev-other test-other train-clean-100 train-clean-360 train-other-500; do
    #         local/download_and_untar.sh ${LIBRISPEECH} ${data_url} ${part}
	# done
        ...
        log "stage 1: ${MYST}/data is already existing. Skip data downloading"
    fi
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    if [ ! -e "${MYST}/myst_clean_train.csv" ]; then
        log "stage 2: Split is not found... Generating a new split."

        python3 local/create_csv.py -R ${MYST}
        python3 local/clean_csv.py -C ${MYST}/myst.csv    
        python3 local/split_csv.py -C ${MYST}/myst_clean.csv
    else
        log "stage 2: Split is already existing."
    fi 
    python3 local/data_prep.py ${MYST}
    
    for part in train_myst dev_myst test_myst; do
        sort data/${part}/utt2spk -o data/${part}/utt2spk
        sort data/${part}/text -o data/${part}/text
        sort data/${part}/wav.scp -o data/${part}/wav.scp
        utils/utt2spk_to_spk2utt.pl data/${part}/utt2spk > data/${part}/spk2utt
        utils/validate_data_dir.sh data/${part} --no-feats
    done
fi

if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    log "stage 3: combine all training and development sets"
    utils/combine_data.sh --extra_files utt2num_frames data/${train_set} data/train_clean_100 data/train_clean_360 data/train_other_500 data/train_myst
    utils/combine_data.sh --extra_files utt2num_frames data/${train_dev} data/dev_clean data/dev_other data/dev_myst
fi
